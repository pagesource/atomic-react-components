...
describe('<Tag />', () => {
  ...
  test('should render provided text', () => {
    expect(TagComponent.find('.text-content').text()).toEqual('Test');
  });

  test('should control remove button availability through prop', () => {
    expect(TagComponent.exists('.remove-tag')).toEqual(false);
    TagComponent.setProps({ variation: 'removable' });
    expect(TagComponent.exists('.remove-tag')).toEqual(true);
  });

  test('should call parent method on remove if available', () => {
