...
describe('<Video />', () => {
  ...
  it('should not be autoplay and should be mute', () => {
    const VideoComponentRender = render(videoContainer);
    expect(
      VideoComponentRender.find({ autoplay: defaultConfig.autoPlay }) &&
        VideoComponentRender.find({ muted: defaultConfig.muted })
    ).toBeTruthy();
  });

  it('should be a className videoContainer', () => {
    expect(VideoComponent.find('video').hasClass('videoContainer')).toBeTruthy();
  });
