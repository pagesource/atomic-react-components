  ...
  beforeEach(() => {
    wrapper = mount(
      <TableVanilla data={data}>
        <TableVanilla.Column dataMappingKey="name" render={{ content: 'Dessert (100g serving)' }} />
        <TableVanilla.Column dataMappingKey="calories" render={{ content: 'Calories' }} />
        <TableVanilla.Column dataMappingKey="fat" render={{ content: 'Fat (g)' }} />
        <TableVanilla.Column dataMappingKey="carbs" render={{ content: 'Carbs (g)' }} />
        <TableVanilla.Column dataMappingKey="protein" render={{ content: 'Protein (g)' }} />
      </TableVanilla>
    );
    ...
    rows.forEach((tr, rowIndex) => {
      ...
      expect(
        cells
          ...
          .children()
          .prop('value')
      ).toEqual(data[rowIndex].name);
      expect(
        cells
          .at(1)
          .children()
          .prop('value')
      ).toEqual(data[rowIndex].calories);
      expect(
        cells
          .at(2)
          .children()
          .prop('value')
      ).toEqual(data[rowIndex].fat);
      expect(
        cells
          .at(3)
          .children()
          .prop('value')
      ).toEqual(data[rowIndex].carbs);
      expect(
        cells
          .at(4)
          .children()
          .prop('value')
      ).toEqual(data[rowIndex].protein);
    });
  });
});
