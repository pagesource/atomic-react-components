...
describe('Column', () => {
  let innerWrapper;
  beforeEach(() => {
    innerWrapper = shallow(<TableVanilla.Column label="Dessert (100g serving)" name="name" />);
  });
  it('should render correctly', () => {
    expect(innerWrapper).toMatchSnapshot();
  });
  it('should have em tag', () => {
    expect(innerWrapper.find('em')).toBeTruthy();
  });
  it('should have label value', () => {
    innerWrapper.setProps({ intent: 'header' });
    const em = innerWrapper.find('em');
    expect(em.text()).toEqual('Dessert (100g serving)');
  });
});
