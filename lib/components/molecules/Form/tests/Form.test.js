...
describe('Form Component', () => {
  ...
  test('should render DateSelector field', () => {
    const userFormDateSelectProps = {
      format: 'ddmmyy',
      dateLabel: 'Date',
      monthLabel: 'Month',
      yearLabel: 'Year',
    ...
    const DateSelectForm = (
      <Form {...userFormProps}>
        <div>
          <Form.DateSelector {...userFormDateSelectProps} />
        </div>
        <ButtonVanilla primary type="submit">
          Submit
        </ButtonVanilla>
    ...
    const Wrapper = shallowWithTheme(DateSelectForm);
    expect(Wrapper).toMatchSnapshot();
  });
});
