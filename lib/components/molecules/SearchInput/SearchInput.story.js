...
storiesOf('Molecules', module).addWithChapters('SearchInput', {
  chapters: [
    ...
    {
      title: 'SearchInput with children',
      sections: [
        {
          title: 'With Children View',
          sectionFn: () => (
            <SearchInput {...stylesConfig}>
              {({ inputValue, isListening, speechRecognition }) => (
                <>
                  <div>{inputValue}</div>
                  <div>
                    isListening:
                  ...
                  {isListening && (
                    <button onClick={() => speechRecognition.abort()}>Stop Listening</button>
                  )}
                </>
              )}
            </SearchInput>
