...
class Tooltip extends PureComponent<Props, State> {
  ...
  tooltipCont: { current: null | HTMLElement } = null;

  calculatePosition = memoize((left, top, height) => {
    const { body } = document;
    const bodyBoundaries = body.getBoundingClientRect() || { y: 0 };
    const containerWidth = body.clientWidth || 0;
    const tooltipWidth = 200;
    let x = left;
    if (x + tooltipWidth > containerWidth) {
      x = containerWidth - tooltipWidth - 2;
    }
    const y = top - bodyBoundaries.y - height;
    this.setState({ topPos: y, leftPos: x });
  });
