 ...
 */
const Step = ({ children, currentItemtStatus }: StepProps): Node => {
  return (
    <li className={currentItemtStatus}>
      <span>{children}</span>
    </li>
  );
};
...
const ProgressIndicator = ({
   ...
   */
  const progressIndWidth = (((currentStep - 1) * 100) / (!!count && count)).toFixed(2);
  ...
  const renderChildren = () => {
    return React.Children.map(children, (step, index) => {
      const currentItemtStatus = getStatus(currentStep, index);
